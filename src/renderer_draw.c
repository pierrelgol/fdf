...
void renderer_from_world_to_projected(t_renderer *const self, const int32_t world_width, const int32_t world_height, const int32_t screen_width, const int32_t screen_height)
{
    ...
    for (int y = 0; y < world_height; ++y)
    {
        for (int x = 0; x < world_width; ++x)
        {
            ...
            double projected_y = (relative_x + relative_y) * sin30 - relative_z;
...
void renderer_setup(t_renderer *const self, const t_fdf_container *const fdf)
{
	...
	renderer_from_world_to_projected(self, self->world_width, self->world_height, WIDTH, HEIGHT);
}
...
t_color color_lerp(t_vec2 p, t_vec2 start, t_vec2 end, t_color color_start, t_color color_end)
{
...
void draw_line(t_renderer *const self, t_vec2 p0, t_vec2 p1, const t_vec2 c)
{
	...
	while (1)
	{
		draw_pixel(self, p, color_lerp(p, p0, p1, color(c.u), color(c.v)).color);
...
void renderer_teardown(t_renderer *const self, const t_fdf_container *const fdf)
{
	(void) self;
	(void) fdf;
	return;
